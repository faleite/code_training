def f_contar_c(s):  # criar função que conta casacteres da string 's'(s)
    ...
    f_def_c_ant = f_c_ordenados[
        0]  # [0] = repr. 1º c #manter ref p c anterior da s enqto o c anterior for = ao c corrente, somar num de vez q esse c aparece
    valor_c = 1
    for c in f_c_ordenados[
             1:]:  # iteração dos c subsequntes, q é a partir de [1] (no caso o 1º c é o [0], o 2º c é [1])
        if c == f_def_c_ant:
            valor_c += 1
